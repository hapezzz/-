...
<c:if test="${like_flag==0 }"><a href="${pageContext.request.contextPath}/like/${requestScope.article.getArticle_id()}">点赞</a></c:if>
<c:if test="${like_flag==1 }">你已经赞过该文章了</c:if>
<br>
<c:if test="${report_flag==0 }"><a href="${pageContext.request.contextPath}/report/${requestScope.article.getArticle_id()}">举报</a></c:if>
<c:if test="${report_flag==1 }">你已经举报过该文章了</c:if>
<br>
点赞数：${requestScope.like_num }    被举报数：${requestScope.report_num }
<form action="${pageContext.request.contextPath}/comment" >
输入你的评论：<input type="text" name="content"/>
<input type="hidden" name="art_id" value="${requestScope.article.getArticle_id() }"/>
